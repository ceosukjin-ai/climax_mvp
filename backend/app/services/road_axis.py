...
# 도로축 OSM(Overpass) 하드 타임아웃 [초]. 초과하면 가정값(assumed)으로 폴백해
# 실시간 응답을 보장한다. Overpass 정상 응답은 ~1.2s 라 2.0s면 대개 통과하고,
# 7~56s 로 폭주하는 병리적 경우만 잘라낸다. (값 조정은 여기 한 곳)
OSM_TIMEOUT_SEC = 2.0
...
async def get_road_axis(
    ...
    *,
    ...
    """
    # ① OSM (Overpass) — 실시간 응답 위해 하드 타임아웃; 초과 시 가정값 폴백.
    try:
        return await asyncio.wait_for(
            road_axis_from_osm(lat, lon, radius_m=radius_m, client=client),
            timeout=OSM_TIMEOUT_SEC,
        )
    except (RoadAxisError, asyncio.TimeoutError, httpx.HTTPError) as e:
        osm_err = (
            f"{OSM_TIMEOUT_SEC:.1f}s 타임아웃"
            if isinstance(e, asyncio.TimeoutError) else str(e)
        )
        logger.warning("도로축 OSM 실패/타임아웃 → fallback: {}", osm_err)
