...
#
# 채택값: 반경 30 m / 최소수고 3 m / **τ 0.35**
#   반경 25/35/45 m 가 소수 셋째자리까지 동일 -> 25 m 밖 나무는 건물에 가려 기여가 없다 -> 30 m 고정.
#   최소수고 2 m 는 관목·건물 가장자리 잡음일 가능성이 높고 눈높이 1.5 m 에서 하늘을 막지도 못한다.
#
#   τ 는 **엔진에서 직접 훑어** 정했다(환경변수라 재배포 없이 스캔된다).
#       τ      3월MAE  3월bias   8월MAE  8월무목  8월트인
#      0.45    0.145   +0.063    0.108   0.081    0.109
#      0.40    0.141   +0.040    0.108   0.080    0.112
#      0.35    0.138   +0.017    0.108   0.080    0.115   <- 안쪽 최소
#      0.30    0.139   -0.005    0.109   0.080    0.118
#      0.25    0.149   -0.028    0.110   0.080    0.121
#   경계가 아니라 안쪽 최소이고, 그 자리에서 3월·8월 bias 가 둘 다 0 에 붙는다(+0.017/+0.020).
#   대조군(8월 나무없는 44지점)은 τ 전 구간에서 0.080 으로 통과한다.
#   전체 106지점 가중 MAE 도 τ=0.35 가 최소다(0.1156).
#
#   ⚠️ 적합 스크립트(`scripts/fit_canopy_svf.py`)가 내놓은 τ 0.55 는 **옛 zenith 규칙 기준**이라
#      지금은 무효다. edge 규칙은 화소당 차폐가 약해 τ 를 더 내려 보상해야 한다.
#
#   1-τ = 0.65 차폐. 잎 투과 10~20% x 화소 점유율 약 0.75 와 자릿수가 맞는다 — 0.45 는 나무치고 성겼다.
#
#   도입 효과(τ=1.0 = 수관 안 봄 대비):
#       3월 PNU 27   MAE 0.314 -> 0.138   bias +0.310 -> +0.017   r 0.304 -> 0.643
#       8월 부산 79  MAE 0.118 -> 0.108   bias +0.050 -> +0.020   r 0.637 -> 0.682
#       8월 무목 44  MAE 0.081 -> 0.080   <- 대조군 무손상
#   LOO(적합 스크립트, zenith 기준) 0.171 -> 0.129 — 과적합이 아니다.
#
...
# 앱 측정은 실시간 계산이 기본(2026-09-12). 격자는 양자화로 MAE 0.121→0.153.
# 경로·코스 기능은 skyline.get_cells 를 직접 쓰므로 영향 없음.
_USE_GRID = os.environ.get("GEO_USE_GRID", "0") == "1"
