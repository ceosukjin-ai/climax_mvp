...
class MRTConfig:
    ...
    # 2026-09-08 실측 교정: 청천(Ineichen) DNI가 여름 부산 연무·박무의 직달을
    #   과대추정 → 양지 MRT +10°C 편향. 실측 80점(expose 그늘 반영) 대비 DNI×0.6에서
    #   bias +6.7→−1.5, MAE 10.0→6.5(정답지 노이즈 바닥 MAE 5.95에 근접). 빔(직달)에만
    #   적용, 산란(DHI)·장파 불변. 되돌리려면 1.0.
    dni_turbidity: float = 0.6   # 직달일사 탁도 감쇠 계수 [0,1] (연무 보정)
    # 2026-09-09 실측 교정(지면 열관성): 그늘 지점의 실측 지표온도가 양지 대비 겨우 −5°C
    #   (실측 Ts 그늘 49.3 / 양지 54.2, n=21/59)인데 정상상태 에너지수지는 그늘에서 Ta+1(35.6)로
    #   떨어뜨려 그늘 Tmrt −7.7°C·PET −4.8°C 편향의 주원인. 그늘은 하루 중 옮겨 다니고 지면은
    #   열용량이 커서 직달이 끊겨도 승온분의 대부분을 유지 → 직달 차단분의 이 비율만큼 유지.
    ...
    # 2026-09-05 1차 실측 교정: 부산 5개 지역 열화상 표면온도 80점 회귀.
