...
# LCZ 는 **색과 모양을 함께** 쓴다 — 색만 쓰면 흑백 인쇄에서 사라지고, 모양만 쓰면
# 작은 표식이라 구분이 더디다. 기기(계측기)는 띠·기준선·기기 이름 색으로 남는다.
#   2026-09-20: 다섯 색이 모두 진해 표식이 무겁게 보였다. 한 단계씩 연하게 내리고
#   LCZ 5 를 가장 연하게 둔다 — 모양이 이미 구분을 지고 있으므로 색은 거들기만 하면 된다.
LCZ = [("LCZ 1", "부암제1동", "Buam 1",     "o", 11.0, "#4E8EC4"),
       ("LCZ 2", "보수동",   "Bosu",       "s",  9.0, "#45B5AF"),
       ("LCZ 3", "서제2동",  "Seo 2",      "^", 12.0, "#93BC5E"),
       ("LCZ 4", "용호제1동", "Yongho 1",   "D",  7.5, "#EE8E56"),
       ("LCZ 5", "명장동",   "Myeongjang", "v", 12.0, "#B3A3D6")]
CODE = {d: (c, mk, sz, col) for c, d, _en, mk, sz, col in LCZ}
...
def load():
    ...
    for r in csv.DictReader(open(f"{DATA}/tier3_engine_output_80_v7_photo.csv",
        ...
        rows.append(dict(lcz=CODE[r["권역"]][0], mk=CODE[r["권역"]][1],
                         sz=CODE[r["권역"]][2], col=CODE[r["권역"]][3],
...
def main():
    ...
    for ax, (title, inst, ic, key, unit, dec) in zip(axes.flat, PANELS):
        xs = [(r[key], r["lcz"], r["mk"], r["sz"], r["col"])
              for r in rows if r[key] is not None]
        v = sorted(x for x, _l, _m, _s, _c in xs)
        med = st.median(v)
        ...
        for x, _l, mk, sz, col in xs:
            ax.scatter([x], [rng.uniform(-0.28, 0.28)], s=sz, marker=mk, color=col,
                       zorder=3, lw=0)
    ...
    fig.text(0.04, 0.95,
             "Same site, same minute. Each marker is one site; shape and colour both give the "
             "local climate zone, while the band, baseline and instrument label carry the "
             "instrument. Black line = median, band = interquartile range. "
             "View factors re-derived on 2026-09-14 (fisheye stitched, tilted cube-map; "
             "79 sites — one pose failure excluded).",
             fontsize=6.8, color=MUTED)
    fig.legend(handles=[Line2D([], [], marker=mk, color="none", markerfacecolor=col,
                               markersize=4.8, label=f"{code} · {en}")
                        for code, _d, en, mk, _sz, col in LCZ],
