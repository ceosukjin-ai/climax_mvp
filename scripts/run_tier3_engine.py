...
IN = "data/tier3_engine_input_80.csv"
OUT = "data/tier3_engine_output_80_v2.csv"
...
def run(lat, lon, when, svf, gvi, ta, rh, v, shade, ped=False):
    ...
    r = compute_vpti_thermal(views_5=views(svf, gvi), materials=MATS, weather=wc,
                             ...
                             direct_shade=shade, wind_is_pedestrian=ped)
...
def main():
    ...
    extra = ["run_A_Tmrt", "run_A_PET", "run_B_Tmrt", "run_B_PET",
    ...
    for r in rows:
        ...
        a_t, a_p = run(lat, lon, when, svf_app, gvi_app, ta_a, rh_a, v_a, shade)  # A: 앱기상+앱시계
        b_t, b_p = run(lat, lon, when, svf_t3, gvi_t3, ta_a, rh_a, v_a, shade)    # B: 앱기상+tier3
        c_t, c_p = run(lat, lon, when, svf_t3, gvi_t3, ta_m, rh_m, v_m, shade, True)    # C: 실측기상+tier3
        d_t, d_p = run(lat, lon, when, svf_pano, gvi_pano, ta_m, rh_m, v_m, shade, True)  # D: 실측기상+파노
        nr = dict(r)
        nr.update({"run_A_Tmrt": a_t, "run_A_PET": a_p, "run_B_Tmrt": b_t, "run_B_PET": b_p,
