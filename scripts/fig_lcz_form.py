...
LCZ = [("부암제1동", "1",   (25, None), (2.0, None), (0.2, 0.4)),
       ("보수동",   "2",     (10, 25),   (0.75, 2.0), (0.3, 0.6)),
       ("서제2동",  "3",     (3, 10),    (0.75, 1.5), (0.2, 0.6)),
       ("용호제1동", "4",   (25, None), (0.75, 1.25), (0.5, 0.7)),
       ("명장동",   "5", (10, 25),   (0.3, 0.75), (0.5, 0.8))]
...
fig, axes = plt.subplots(1, 3, figsize=(140 * MM, 58 * MM))
fig.subplots_adjust(left=0.06, right=0.99, top=0.86, bottom=0.30, wspace=0.22)
for ax, (title, col, k, ylim, yt) in zip(axes, panels):
    for i, (dong, lab, *rngs) in enumerate(LCZ):
        ...
        ax.scatter([i], [med], s=34, color=INK, edgecolor="none", zorder=4)
        ax.text(i + 0.14, med, f"{med:.1f}" if col == "H" else f"{med:.2f}", ha="left", va="center", fontsize=7, color=INK, zorder=5)
        print(f"{title[:4]} {dong:6s} n {len(q):2d} median {med:.3f}  class {lo}-{hi}  {'in' if inside else 'OUTSIDE'}")
    ax.set_xlim(-0.6, 4.6); ax.set_ylim(*ylim); ax.set_yticks(yt)
    ax.set_xticks(range(5)); ax.set_xticklabels([l for _, l, *_ in LCZ], fontsize=7.5)
...
fig.legend(handles=[Rectangle((0, 0), 1, 1, facecolor=SURF, edgecolor="none", label="range defining the assigned LCZ class (Stewart and Oke, 2012)"),
                                        Line2D([], [], marker="o", color="none", markerfacecolor=INK, markersize=5, markeredgecolor="none", label="median of the measurement sites")],
           loc="lower center", bbox_to_anchor=(0.5, 0.02), ncol=2, frameon=False, fontsize=7, handletextpad=0.5, columnspacing=1.8)
