...
sys.path.insert(0, "/mnt/user-data/uploads/climax_mvp/backend")
...
R = list(csv.DictReader(open(f"{BASE}/tier3_engine_output_80_v6.csv", encoding="utf-8-sig")))
...
phys = np.array([float(LO[r["측정ID"]]["물리_PET"]) for r in R])
ai = np.array([float(LO[r["측정ID"]]["물리+AI_Ridge_LOSO_PET"]) for r in R])
...
PANELS = [
    ...
    ("e", "Tier 3 ++", "+ sun / shade observed\nat the site", c("run_E_PET"), None),
    ("f", "Deployed engine", "+ recalibrated physics\n+ ridge residual (LOSO)", ai, phys),
]
...
fig.savefig("/tmp/claude-0/SCS_Fig_tier_ladder.pdf", bbox_inches="tight", pad_inches=0.02)
fig.savefig("/tmp/claude-0/SCS_Fig_tier_ladder.png", bbox_inches="tight", pad_inches=0.02)
print("saved")
