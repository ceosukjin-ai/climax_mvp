...
one() {  # one 이름 S W N E
  ...
  osmium extract -b $W,$S,$E,$N "$PBF" -o $t.osm.pbf --overwrite || { echo "  extract 실패"; return; }
  osmium tags-filter $t.osm.pbf w/building a/building -o $t.b.osm.pbf --overwrite
  osmium export -f geojsonseq --add-unique-id=type_id $t.b.osm.pbf -o $t.geojsonl --overwrite
  ...
  osmium tags-filter $t.osm.pbf n/natural=tree w/natural=tree_row -o $t.t.osm.pbf --overwrite
  osmium export -f geojsonseq --add-unique-id=type_id $t.t.osm.pbf -o $t.t.geojsonl --overwrite
