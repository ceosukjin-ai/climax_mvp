...
# ---------------------------------------------------------------- (a) wall vs ground
rows = list(csv.DictReader(open("/mnt/user-data/uploads/climax_mvp/data/thermal_wall_23.csv", encoding="utf-8-sig")))
...
# ---------------------------------------------------------------- (b) globe - body
import pandas as pd
_B = pd.read_csv("/mnt/user-data/uploads/climax_mvp/data/tier3_80_body_260913.csv")[["측정ID","태양고도"]]
_B = _B.merge(pd.read_csv("delta_v7_final.csv"), on="측정ID").merge(pd.read_csv("tier3_engine_output_80_v7_photo.csv")[["측정ID","볕"]], on="측정ID")
_B["정의차_흑구−전신"] = _B.dglobe
el, dl, sun = _B.태양고도.values.astype(float), _B["정의차_흑구−전신"].values.astype(float), (_B.볕.values == 1)
el, dl, sun = np.array(el), np.array(dl), np.array(sun)
ax = axes[1]
softgrid(ax, axis="y")
ax.scatter(el[sun], dl[sun], s=24, facecolor=ORANGE, edgecolor="white", lw=0.5,
           marker="o", zorder=4, label="Sunlit (photo-verified)")
ax.scatter(el[~sun], dl[~sun], s=26, facecolor=PURPLE, edgecolor="white", lw=0.5,
           marker="^", zorder=4, label="No direct beam")
for m, c, lab, dy in ((sun, ORANGE, "Sunlit", 1.15), (~sun, PURPLE, "No beam", -1.45)):
...
# ---------------------------------------------------------------- (c) model ranking
cfg = [
    ("Wall = pavement (deployed)",   5.16, 4.63),
    ("+ wall shortwave reflection",  4.91, 4.33),
    ("+ resolved wall temperature",  3.47, 1.97),
    ("+ wall thermal mass",          3.31, 1.62),
]
...
fig.savefig("fig/out_v7/SCS_Fig_wall_globe.pdf", bbox_inches="tight", pad_inches=0.02)
fig.savefig("fig/out_v7/SCS_Fig_wall_globe.png", bbox_inches="tight", pad_inches=0.02)
print("saved")
